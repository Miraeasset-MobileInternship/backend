...
//    @Operation(description = "accessToken이 만료되어 401 에러를 받은 경우, 보유한 리프레스 토큰으로 갱신 요청")
    ...
    //로그인 로그아웃 - withoutClassInfo (프론트 요청으로 제작 but 사용되지 않음)
//    @PostMapping("/start-signin-without-classinfo")
////    @Operation(description = "시작하기/로그인 - 가입진행된 유저일 경우(바로 로그인됨), 가입안된 유저의 경우(303 : 가입진행창으로 넘어가게)")
//    @Operation(summary = "로그인, 시작하기", description = "로그인기능",
//            responses = {
//                    @ApiResponse(responseCode = "E000", description = "Success", content = @Content(schema = @Schema(implementation = SignInResponseDtoV2.class))),
//                    @ApiResponse(responseCode = "E101", description = "핸드폰 인증번호 맞지 않음", content = @Content ),
//                    @ApiResponse(responseCode = "E306", description = "회원가입이 진행되지 않은 유저(이름 입력페이지로 넘어가 유저 정보로 가입진행되어야 함)", content = @Content ),
//                    //로그인에서 발생할 수 있는 에러
//                    @ApiResponse(responseCode = "E303", description = "권한이 없는 사용자", content = @Content ),
...
//                    @ApiResponse(responseCode = "E404", description = "존재하지 않는 학급", content = @Content ),
//            })
//    public ResponseEntity getStartWithSignInWithoutClassInfo(@RequestBody @Valid SignInRequestDto signInRequestDto){
//        return ResponseEntity.ok(authService.getStartWithoutClassInfo(signInRequestDto));
//    }


//    @PostMapping("/start-signup-without-classinfo")
//    @Operation(summary = "회원가입 및 자동로그인", description = "",
//            responses = {
//                    @ApiResponse(responseCode = "E000", description = "Success", content = @Content(schema = @Schema(implementation = SignInResponseDtoV2.class))),
//                    //회원가입 발생 에러
...
//                    @ApiResponse(responseCode = "E404", description = "존재하지 않는 학급", content = @Content ),
//            })
//    public ResponseEntity getStartWithSignUpWithoutClassInfo(@RequestBody @Valid SignUpRequestDto signUpRequestDto){
//        return ResponseEntity.ok(authService.startWithSignUpWithoutClassInfo(signUpRequestDto));
//
//    }



    @GetMapping("/{user_id}/get-token")
//    @Operation(description = "속해 있는 모든 학급 리스트를 반환 - 학생용")
    @Operation(summary = "accessToken얻기", description = "userId를 이용해 accessToken얻기",
            responses = {
                    @ApiResponse(responseCode = "E000", description = "Success", content = @Content(array = @ArraySchema(schema = @Schema(implementation = TokenDto.class)))),
                    @ApiResponse(responseCode = "E403", description = "존재하지 않는 유저", content = @Content ),
            })
    public ResponseEntity<BanklassResponseEntity> getAccessToken(@PathVariable(value = "user_id") Long userId){
        return ResponseEntity.ok(authService.loginForWebView(userId));
    }
